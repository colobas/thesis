...
class VariationalMixture(nn.Module):
    def __init__(self, xdim, hdim, n_hidden, n_classes, components):
        ...
        pretrain_decoder_modules = (
            sum([[nn.Linear(hdim, hdim), nn.ReLU(), nn.BatchNorm1d(hdim)] for i in range(n_hidden)], []) +
            [nn.Linear(hdim, xdim)]
        )

        self.encoder = nn.Sequential(*net_modules)
        self.pretrain_encoder = nn.Sequential(*(net_modules[:-2]))
        self.pretrain_decoder = nn.Sequential(*pretrain_decoder_modules)
    ...
    def pretrain_loss(self, x):
        return ((x - self.pretrain_decoder(self.pretrain_encoder(x)))
                    .pow(2)
                    .sum(dim=1)
                    .mean())
    ...
    def fit(self, X, dataloader, n_epochs=1, opt=None,
            ...
            verbose=False, writer=None, pretrain=False):
        ...
        for epoch in epochs:
            for i, xb in enumerate(dataloader):
                ...
                if pretrain:
                    ...
                    loss.backward()
                    opt.step()
                    if (writer is not None) and (n_iter % 5 == 0):
                        writer.add_scalar('losses/pretrain', loss, n_iter)
...
pretrain_opt = optim.Adam(
    list(mixture.pretrain_encoder.parameters()) + 
    list(mixture.pretrain_decoder.parameters()),
    lr=1e-2
)


# %%
def pretrain(n_epochs, bs, mixture):
...
# %%
pretrain(n_epochs=200, bs=256, mixture=mixture)
...
# %%
(X - mixture.pretrain_decoder(mixture.pretrain_encoder(X))).pow(2).sum(dim=1).mean()
