...
def gen_samples(batch_size=512):
    z_dist = distrib.Normal(loc=0, scale=5)
    z_samples = z_dist.sample((batch_size,))
    
    f = lambda z: torch.sigmoid(np.pi * z)
    
    return f(z_samples)

x_samples = gen_samples(2048)

# %%
f, ax = plt.subplots(1, 1, figsize=(10, 10))

inverse = lambda x: torch.log(x/(1-x))/np.pi
z_dist = distrib.Normal(loc=0., scale=5.)

def true_likelihood(x):
    return (1/(np.pi * (x - x**2))) * z_dist.log_prob(inverse(x)).exp()

xx = np.arange(0.01, 1.0, 0.01)
yy = true_likelihood(torch.Tensor(xx)).numpy()

ax.plot(xx, yy, linewidth=3, color="green")

_ = plt.xlim(0, 1)

# %%
base_dist = distrib.Normal(loc=0., scale=1.)
...
for it in trange(int(1e5)):
    ...
    if it % 5000 == 0:
        with torch.no_grad():
            ...
            yy = flow.final_density.log_prob(xx).exp().numpy()
            plt.plot(xx.numpy(), yy, linewidth=3, color="red")
            
            yy = true_likelihood(xx).numpy()
            plt.plot(xx.numpy(), yy, linewidth=3, color="green")
            #plt.xlim(0, 60)
            #plt.ylim(-15, 15)
            plt.show()
